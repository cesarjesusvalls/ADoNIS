...
| **7** | T2K CC0π δp_T, δα_T (CH) | T2K (NUISANCE ROOT) | ◐ **BIT-EXACT PRIMARY now** (adonis/xsec): CC0π = spectral-fn QE + RES-π-absorbed (6% of CC0π) at first-principles absolute weights, no fitted mix. ⟨δp_T⟩ 187.6 vs ACH 196 (4%), rms 246 vs 250 (2%), ⟨δα_T⟩ 98.3 vs 96.3. Cylinder cascade (run-card-correct) + 132k events: per-bin <=16% (dpt)/17% (dalphat), down from 20/23% Gaussian. Primary CONFIRMED faithful (no-FSI <dpT> 131.9 vs ACH 135.6, +3%); cascade overall-rate faithful (+42 vs +44%). Residual = cascade SHAPE (faithful Cascade.cc port: formation zones, ClosestApproach geometry). |
