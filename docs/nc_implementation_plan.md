...
| **`run_ee_*_fsi` cards route to the emulated amd64 `:oracle` image while running the cascade** | **Found while writing the NC routing test, and deliberately NOT asserted as one.** `run_achilles._RULES` has no prefix for them, so `_image_for` returns the amd64 default. Whether that is a latent SIGSEGV or those cards are simply never driven through this runner is **not established** — the (e,e′) banks exist, so something works. Recorded as a question, not a claim. The NC test is scoped to the NC card only. |
...
| **NC QE nuclear sampler** | **~1.8 % of the Fig 11/12 signal** | I first claimed NC QE "cannot enter an NC1π⁰ signal". **Measured on the ACHILLES reference: 516 of 29 194 selected NC1π⁰ events (1.77 %) are proc 250/251, i.e. NC ELASTIC** — the cascade makes the π⁰ by charge exchange on the recoiling nucleon. Real physics the `channels: [res]` bank misses. Expect ADoNIS ~1.8 % low on the NC1π⁰ rate from this cause alone. Recorded so the deficit is recognised rather than tuned away or misread as a current-level disagreement. |
