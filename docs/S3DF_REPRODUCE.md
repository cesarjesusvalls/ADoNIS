...
- **ADoNIS banks (all probes)** — ONE CLI, `adonis.workflow.cli <config> --out <dir>`; a shard is one
  ...
  `... $ADONIS_PY -u -m adonis.workflow.cli configs/banks/nu_T2K_C.yaml --n-per-seed 250000
  --n-seeds 1 --seed0 $SLURM_ARRAY_TASK_ID --out $ADONIS_OUT/nu_T2K_C_10M/part_$SLURM_ARRAY_TASK_ID`.
  Beam banks (`configs/banks/beam_*.yaml`, probe=hadron) and (e,e') banks (`configs/banks/beam_e_*.yaml`, probe=EM)
