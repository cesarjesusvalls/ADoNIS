...
def _obs(mu, lead, pip, knu=None):
    ...
    return {"dpt": np.asarray(BP.dpt_1pi(mu, lead, pip)),
            "dalphat": np.asarray(BP.dat_1pi(mu, lead, pip)),
            "dphit": np.asarray(BP.dphit_1pi(mu, lead, pip)),
            "pn": np.asarray(BP.pN_1pi(mu, lead, pip)),
            "dptt": np.asarray(BP.dptt_1pi(mu, lead, pip)),
            "pmu": pmu, "cos_mu": cmu, "th_mu": np.degrees(np.arccos(np.clip(cmu, -1.0, 1.0))),
            "th_mu_rad": np.arccos(np.clip(cmu, -1.0, 1.0)),

            "pt": np.sqrt(mu[:, 1] ** 2 + mu[:, 2] ** 2), "pz": mu[:, 3],
...
def _cc_full(B, sd):
    ...
    if sd.pion_id == "none":
        ...
        if sd.pt_hi is not None:
            sel = sel & (np.sqrt(mu[:, 1] ** 2 + mu[:, 2] ** 2) <= sd.pt_hi)
        if sd.pz_win is not None:
...
def _ele_full_bank(B, sd):
    ...
    if getattr(sd, "omega_win", None) is not None:
...
_EIDX_FAM = ("f_p_eidx", "f_n_eidx", "ks_eidx")
...
def oracle_signal_nc(oracle_npz, sd):
    ...
    chan = np.isin(proc, (451, 452)).astype(int)
    sel = ((npi0 == 1) & (npip == 0) & (npim == 0) & (n_heavy == 0)
           & (ppi >= sd.pi_win[0]) & (ppi < sd.pi_win[1]) & (cpi > sd.cth))
    if sd.proton_count == "eq1":
        sel = sel & hasp
...
def ele_oracle_signal(oracle_npz, sd):
    ...
    return _ele_finish(obs, elec, w, np.zeros(n, int), npi, nprot)
