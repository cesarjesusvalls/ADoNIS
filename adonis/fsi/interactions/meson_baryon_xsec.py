...
_CHANNELS = {
    (0, "p"): [(0, "p", {3: 1.0})],
    (0, "n"): [(0, "n", {3: 1.0 / 3, 1: 2.0 / 3}),
               (1, "p", {3: _R2, 1: -_R2})],
    (1, "p"): [(1, "p", {3: 2.0 / 3, 1: 1.0 / 3}),
               (0, "n", {3: _R2, 1: -_R2})],
    (1, "n"): [(1, "n", {3: 2.0 / 3, 1: 1.0 / 3}),
               (2, "p", {3: _R2, 1: -_R2})],
    (2, "p"): [(2, "p", {3: 1.0 / 3, 1: 2.0 / 3}),
               (1, "n", {3: _R2, 1: -_R2})],
    (2, "n"): [(2, "n", {3: 1.0})],
}
...
def _build_table():
    ...
    for (pin, nuc), outs in _CHANNELS.items():
        for (pout, nout, cg) in outs:
            grid[(pin, nuc, pout)] = _channel_sigma(amps, Wt, cg)
    _TABLE["grid"] = grid
    return _TABLE
...
def channel_sigmas_pn_avg(W, pion_in_idx):
    ...
    for nuc in ("p", "n"):
        for (pout, nout, cg) in _CHANNELS[(pion_in_idx, nuc)]:
            ...
            out[pout] += 0.5 * np.clip(s, 0.0, None)
    return out
...
def _build_angular():
    ...
    Wg = np.linspace(1085.0, 2200.0, 224)
    cg_grid = np.linspace(-1.0, 1.0, 181)
    ug = np.linspace(0.0, 1.0, 64)
        ...
        for nuc in range(2):
            for pout in range(3):
                ...
                for i, W in enumerate(Wg):
                    ...
                    inv[ci, i] = np.interp(ug, cdf, cg_grid)
    _ANG["inv_np"] = inv
...
def jax_conversion_sigma(W, pion_in_idx_arr, nuc_idx_arr):
    ...
    flat = _jnp.stack([_jnp.interp(W, jW, jsig[i, nu], left=0.0, right=0.0)
                       for i in range(3) for nu in range(2)], axis=0)
    sel = (pion_in_idx_arr * 2 + nuc_idx_arr).astype(_jnp.int32)
...
def jax_eta_backconv_sigma(W, nuc_idx_arr):
    ...
    REGENERATES a pion)."""
    _, _, (jW, jsig) = _jax_grid_eta()
    all_np = _jnp.stack([_jnp.stack(
        [_jnp.interp(W, jW, jsig[nu, o], left=0.0, right=0.0) for o in range(3)], axis=-1)
        for nu in range(2)], axis=0)
    return _jnp.take_along_axis(all_np, nuc_idx_arr.astype(_jnp.int32)[None, :, None], axis=0)[0]
