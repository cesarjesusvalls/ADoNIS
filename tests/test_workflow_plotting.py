...
def test_hist_offset_raises_chi2():
    ...
    vals = np.array([0.5, 1.5, 2.5, 3.5, 4.5, 5.5, 6.5, 7.5, 8.5, 9.5])
    ref = {"values": vals, "w": np.ones_like(vals)}
    ado = {"values": vals, "w": 2.0 * np.ones_like(vals)}          # ADoNIS 2x the ref -> chi2 > 0, ratio 0.5
    fig, a0, a1 = _axes()
    r = chi2_ratio_panel(a0, a1, edges, ref, ado, label="x")
    plt.close(fig)
    assert r["chi2"] > 0.0
    assert r["ach_ado"] < 1.0                                      # ref/ado ~ 0.5
...
def test_curve_mode_contract():
    ...
    fig, a0, a1 = _axes()
    r = chi2_ratio_panel(a0, a1, x, ref, ado, label="c")           # "y" in ref -> _curve_panel path
    plt.close(fig)
