"""The numpy reference for GiBUU N N -> N Delta -- the heart
...
def test_production_xsec_physical():
    ...
    is finite/positive."""
    assert nd.sigma_nn2ndelta(2.0) < 0.05
...
def test_fig14_pp_channels():
    ...
    assert 15 < pn.max() < 26, pn.max()
    assert 3 < pp.max() < 6, pp.max()
    assert abs(pn.max() / pp.max() - 5.0) < 0.2
    assert 2.2 < s[pn.argmax()] < 2.6
    assert pn[0] < 0.3 and pn[-1] < pn.max()
