...
padded to K), ALL protons + ALL neutrons (4-vec each, padded to M), struck nucleon 4-vec + pid, nu 4-vec, n_other_meson,
...
MU, PIP, PROT, NEUT = 13, 211, 2212, 2112
PIONS = {111, 211, -211}
...
for evt in parse_events(Path(HEPMC)):
    ...
    if mu is None or nu is None or struck is None:
        continue
    nkept += 1
    # pad pions (4-vec + pid) to K, protons + neutrons (4-vec each) to M
...
np.savez(OUT, mu=np.array(mu_l), nu=np.array(nu_l), struck=np.array(st_l),
         ...
         prot_p4=np.array(pr4_l), neut_p4=np.array(nr4_l), n_other_meson=np.array(nom_l), w=np.array(w_l),
