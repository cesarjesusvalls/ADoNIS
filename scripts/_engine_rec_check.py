"""Stage 3a: cascade_carbon_v2(rec_caps=...) returns the joint per-event FSI record; nominal==1; the
...
cfg = DiscreteCascadeConfig(cylinder=True, step=0.04, max_steps=600, seed=1, nn_inelastic=True, pauli=True,
                            early_exit=True, nucleus=tg.density_p, density_n=tg.density_n,
                            configs=tg.configs, engine="pool")
key = jax.random.PRNGKey(11)
# 4-tuple default unchanged
r4 = CF.cascade_carbon_v2(g("p_pi"), g("p_N"), g("ppid").astype(jnp.int32), g("ipid").astype(jnp.int32),
                          g("Npid").astype(jnp.int32), cfg, key, P=P, max_gen=MG, channel="res")
print(f"[4-tuple default] len={len(r4)}  (expect 4)")
# 5-tuple with record
r5 = CF.cascade_carbon_v2(g("p_pi"), g("p_N"), g("ppid").astype(jnp.int32), g("ipid").astype(jnp.int32),
                          g("Npid").astype(jnp.int32), cfg, key, P=P, max_gen=MG, channel="res",
                          rec_caps=(32, 256))
