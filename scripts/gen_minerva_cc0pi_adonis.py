"""ADoNIS MINERvA CC0pi-Np STV prediction: flux-folded CCQE (Llewellyn-Smith) off the spectral-
...
MINERvA CC0pi cuts: 1500<p_mu<10000 & theta_mu<20deg, leading p 450-1200 & theta_p<70deg.
"""
import os, sys
...
def generate(n=1_000_000, seed=0, with_fsi=True, MA_GeV=1.03):
    flux = Spectrum(REL / "MINERvA" / "MINERvA_cc1pi0_flux.dat")
    ...
    import numpy as _np
    C20=_np.cos(20*_np.pi/180); C70=_np.cos(70*_np.pi/180)
    sel = (wv > 0) & (pmu_f > 1500) & (pmu_f < 10000) & (cmu > C20) & (ppm > 450) & (ppm < 1200) & (cp > C70)
    dpt = np.asarray(obs.delta_pT(ev))[sel]
    dat = np.asarray(obs.delta_alphaT(ev))[sel]
    pn = np.asarray(obs.p_N_tki(ev))[sel]
    return dict(dpt=dpt, dalphat=dat, pn=pn, w=wv[sel])
...
if __name__ == "__main__":
    ...
    for fsi in (False, True):
        ...
        np.savez(ROOT / "data" / "oracle" / f"minerva_cc0pi_tki_adonis_{'fsi' if fsi else 'nofsi'}.npz", **d)
    print("wrote data/oracle/minerva_cc0pi_tki_adonis_{fsi,nofsi}.npz")
