...
```bash
python -m analysis.campaign.constrained --label minimal_jacobian --fit-config configs/fits/minimal.yaml --max-chunks 4
ADONIS_JACOBIAN_NPZ=output/results/minimal_jacobian.npz \
    python -m analysis.campaign.run configs/fits/minimal.yaml --stage closure
```

`configs/fits/minimal.yaml` is the full inference chain at the smallest size that still exercises it.
