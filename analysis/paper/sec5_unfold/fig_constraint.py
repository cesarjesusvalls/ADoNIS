...
def main(label="sec5", study="asimov"):
    ...
    ax.set_title("(a) templates — no prior", fontsize=8.5, loc="left")
    ...
    for k, (blk, ttl, rot, lab) in enumerate(
        ...
        if blk == "flux":
            ...
            ax.set_xticklabels(names[m], rotation=rot, fontsize=6.5)
    ...
    ax.set_title(f"(d) detector — {int(m.sum())} dials", fontsize=8.5, loc="left")
